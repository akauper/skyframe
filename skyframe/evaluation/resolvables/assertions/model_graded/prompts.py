...
DEFAULT_GRADING_PROMPT = """You are grading output according to a user-specified rubric. If the statement in the rubric is true, then the output passes the test. You respond with a JSON object with this structure: {pass: boolean; reason: string;}.
...
OPENAI_FACTUALITY_PROMPT = json.dumps([
    {
        'role': 'system',
...
OPENAI_CLOSED_QA_PROMPT = json.dumps([
    {
        'role': 'system',
...
SUGGEST_PROMPTS_SYSTEM_MESSAGE = json.dumps([
    {
        'role': 'system',
...
REFUSAL_PROMPT_SYSTEM_MESSAGE = json.dumps([
    {
        'role': 'system',
...
SELECT_BEST_PROMPT = """You are comparing multiple pieces of text to see which best fits the following criteria: {{criteria}}
...
AI_SELF_REFERENCE_PROMPT_SYSTEM_MESSAGE = json.dumps([
    {
        'role': 'system',
